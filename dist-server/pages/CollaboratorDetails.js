...
const CollaboratorDetails = ({ id, onBack, onOpenAuditHistory, onUpdate, currentUser, allUsers = [], showToast }) => {
    ...
    const [historicoRefreshKey, setHistoricoRefreshKey] = (0, react_1.useState)(0);
    const fetchDetails = () => fetch(`${config_1.API_URL}/api/users/${id}/details`, { credentials: 'include' })
        .then(r => {
        if (!r.ok) {
            if (r.status === 404)
                    ...
                    fontSize: 14,
                            ...
                            top: 24,
                                    ...
                                    marginBottom: 16,
                                        ...
                                        alignItems: 'center',
                                        gap: 6,
                                    }, children: [user?.isActive && (0, jsx_runtime_1.jsx)("span", { style: { width: 6, height: 6, borderRadius: '50%', background: 'currentColor', animation: 'pulse 1.5s ease-in-out infinite' } }), user?.isActive ? 'Ativo' : 'Inativo'] }) }), (0, jsx_runtime_1.jsxs)("div", { style: { display: 'flex', flexDirection: 'column', gap: 12, fontSize: 14, color: '#e4e4e7' }, children: [(0, jsx_runtime_1.jsxs)("div", { style: { display: 'flex', alignItems: 'center', gap: 10 }, children: [(0, jsx_runtime_1.jsx)(lucide_react_1.Briefcase, { size: 16, color: "#71717a", style: { flexShrink: 0 } }), (0, jsx_runtime_1.jsx)("span", { children: user?.role?.name || user?.jobTitle || '—' })] }), (0, jsx_runtime_1.jsxs)("div", { style: { display: 'flex', alignItems: 'center', gap: 10 }, children: [(0, jsx_runtime_1.jsx)(lucide_react_1.Building2, { size: 16, color: "#71717a", style: { flexShrink: 0 } }), (0, jsx_runtime_1.jsxs)("span", { children: ["Depto: ", user?.departmentRef?.name || '—'] })] }), (0, jsx_runtime_1.jsxs)("div", { style: { display: 'flex', alignItems: 'center', gap: 10 }, children: [(0, jsx_runtime_1.jsx)(lucide_react_1.Building2, { size: 16, color: "#71717a", style: { flexShrink: 0 } }), (0, jsx_runtime_1.jsxs)("span", { children: ["Unidade: ", user?.unitRef?.name || '—'] })] }), user?.role?.code && ((0, jsx_runtime_1.jsxs)("div", { style: { display: 'flex', alignItems: 'center', gap: 10 }, children: [(0, jsx_runtime_1.jsx)(lucide_react_1.Hash, { size: 16, color: "#71717a", style: { flexShrink: 0 } }), (0, jsx_runtime_1.jsx)("span", { children: (user.role.code || '').split(/\s+e\s+/)[0]?.trim() || user.role.code })] })), (0, jsx_runtime_1.jsxs)("div", { style: { display: 'flex', alignItems: 'center', gap: 10 }, children: [(0, jsx_runtime_1.jsx)(lucide_react_1.Mail, { size: 16, color: "#71717a", style: { flexShrink: 0 } }), (0, jsx_runtime_1.jsx)("a", { href: `mailto:${user?.email}`, style: { color: '#38BDF8', textDecoration: 'none' }, children: user?.email || '—' })] }), user?.manager && ((0, jsx_runtime_1.jsxs)("div", { style: { display: 'flex', alignItems: 'center', gap: 10 }, children: [(0, jsx_runtime_1.jsx)(lucide_react_1.User, { size: 16, color: "#71717a", style: { flexShrink: 0 } }), (0, jsx_runtime_1.jsxs)("span", { children: ["Gestor: ", user.manager?.name] })] }))] }), canEdit && isEditModalOpen && currentUser && showToast && ((0, jsx_runtime_1.jsx)(EditUserModal_1.EditUserModal, { isOpen: isEditModalOpen, onClose: () => setIsEditModalOpen(false), user: {
                                    id: user.id,
                                    name: user.name,
                                    email: user.email,
