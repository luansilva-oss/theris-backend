...
async function getCatalogToolsForSlack() {
...
async function pushModalSafely(client, triggerId, view, ctx) {
    try {
...
slackApp.action('btn_move', async ({ ack, body, client }) => {
    await ack();
    try {
        await pushModalSafely(client, body.trigger_id, {
    ...
    try {
        ...
        try {
            ...
            unitOptions = units.length > 0
                ? units.map((u) => ({ text: { type: 'plain_text', text: u.name }, value: u.name }))
                : DEFAULT_UNIT_NAMES.map((name) => ({ text: { type: 'plain_text', text: name }, value: name }));
        }
        catch {
            unitOptions = DEFAULT_UNIT_NAMES.map((name) => ({ text: { type: 'plain_text', text: name }, value: name }));
        }
        await pushModalSafely(client, body.trigger_id, {
...
slackApp.action('acessos_action_type', async ({ ack, body, client }) => {
    ...
    const b = body;
    const view = b.view;
    const selected = (b.actions?.[0]?.selected_option?.value ?? '');
    if (!view?.id || !selected)
        return;
    const actionType = selected;
    const metadata = { actionType };
    const actionLabels = { acesso_extraordinario: 'Acesso Extraordinário', indicar_deputy: 'Indicar Deputy' };
    let toolOptions;
    if (actionType === 'indicar_deputy') {
        ...
        toolOptions = ownedTools.map((t) => ({ text: { type: 'plain_text', text: t.name }, value: t.name }));
    }
    else {
        const catalogTools = await getCatalogToolsForSlack();
        toolOptions = catalogTools.map((t) => ({ text: { type: 'plain_text', text: t.name }, value: t.id }));
    }
    const blocks = [
        {
            type: 'input',
...
function getLevelOptionsFromStaticMap(toolName) {
    const map = TOOLS_AND_LEVELS;
    const normalized = normalizeToolName(toolName);
    // 1) Match exato
    const exact = map[toolName?.trim() ?? ''];
    if (exact?.length)
        return exact.map((item) => ({ text: { type: 'plain_text', text: item.label }, value: item.value }));
    // 2) Match normalizado (ex: "3C Plus" vs "3C PLUS")
    for (const key of TOOL_KEYS) {
        if (normalizeToolName(key) === normalized) {
            const items = map[key];
            if (items?.length)
                return items.map((item) => ({ text: { type: 'plain_text', text: item.label }, value: item.value }));
            break;
        }
    }
    return [];
...
slackApp.action('acessos_tool_select', async ({ ack, body, client }) => {
    ...
    if (actionType === 'indicar_deputy') {
        ...
        toolOptions = ownedTools.length > 0
            ? ownedTools.map((t) => ({ text: { type: 'plain_text', text: t.name }, value: t.name }))
            : [{ text: { type: 'plain_text', text: selectedToolId }, value: selectedToolId }];
    ...
    const blocks = [
        ...
        },
        {
            ...
            element: {
                ...
                placeholder: { type: 'plain_text', text: 'Selecione a ferramenta...' },
                initial_option: {
                    text: { type: 'plain_text', text: actionType === 'acesso_extraordinario' ? selectedToolName : selectedToolName },
    ...
    if (actionType === 'acesso_extraordinario') {
            ...
            if (slackUserId) {
                ...
                if (email) {
                    ...
                    if (userDb?.roleId) {
                        ...
                        const kitItem = role?.kitItems?.find((item) => normalizeForMatch(item.toolName || '') === selectedNorm);
                        if (kitItem) {
        ...
        const descMap = catalogTool?.accessLevelDescriptions ?? null;
        let levelOptions = levelsArray.map((code) => ({
            text: { type: 'plain_text', text: getLevelLabel(code, descMap) },
            value: code
        }));
        ...
        blocks.push({ type: 'input', block_id: 'blk_deputy_name', label: { type: 'plain_text', text: 'Nome do Substituto' }, element: { type: 'plain_text_input', action_id: 'inp' } }, { type: 'input', block_id: 'blk_deputy_email', label: { type: 'plain_text', text: 'E-mail do Substituto' }, element: { type: 'plain_text_input', action_id: 'inp', placeholder: { type: 'plain_text', text: 'email@empresa.com' } } });
    }
    blocks.push({ type: 'input', block_id: 'blk_reason', label: { type: 'plain_text', text: 'Justificativa' }, element: { type: 'plain_text_input', multiline: true, action_id: 'inp' } });
    try {
        await client.views.update({
...
async function saveRequest(body, client, dbType, details, reason, msgSuccess, isExtraordinary = false, actionDate, accessPeriodDays, accessPeriodRaw) {
    try {
        ...
        try {
            ...
            console.log('Erro ao buscar user Slack:', err);
        }
        ...
        if (!requesterId)
            throw new Error("Usuário não encontrado no sistema Theris.");
        let status = 'PENDENTE_SI';
        let currentApproverRole = 'SI_ANALYST';
        let approverId = null;
        let detailsWithMeta = {
            ...details,
        ...
        const accessLevel = details.target || details.targetValue;
        ...
        if (isExtraordinary && toolName) {
            ...
            const { sendAexCreationDMs, sendAexRequesterCreatedDM } = await Promise.resolve().then(() => __importStar(require('./aexOwnerService')));
...
slackApp.action({ action_id: /^aex_owner_(approve|reject)_v2$/, block_id: 'aex_owner_decision' }, async ({ ack, body, client }) => {
    ...
    console.log('[AEX DEBUG] Payload completo:', JSON.stringify({ type: b.type, actions: b.actions?.map((a) => ({ action_id: a.action_id, value: a.value })), user: b.user?.id }, null, 2));
    ...
    try {
        const reqAny = req;
        const toolName = reqAny.toolName || (() => { try {
        ...
        const accessLevel = reqAny.accessLevel || (() => { try {
            const d = JSON.parse(req.details || '{}');
        ...
        if (isApprove) {
            ...
            const { sendAexOwnerApprovedDMs, sendAexRequesterOwnerApprovedDM } = await Promise.resolve().then(() => __importStar(require('./aexOwnerService')));
            ...
            let requesterSlackIdForNotif = null;
            ...
            const { sendAexRejectedByOwnerDM } = await Promise.resolve().then(() => __importStar(require('./aexOwnerService')));
            let requesterSlackId = null;
            if (req.requester?.email) {
                try {
                    const info = await client.users.lookupByEmail({ email: req.requester.email });
...
slackApp.view('acessos_main_modal', async ({ ack, body, view, client }) => {
    ...
    if (!actionType) {
        await client.chat.postMessage({ channel: body.user.id, text: '⚠️ Selecione primeiro uma ação (Acesso Extraordinário ou Indicar Deputy) e preencha os campos.' });
        return;
    }
    ...
    if (actionType === 'acesso_extraordinario') {
        ...
        let accessPeriodDays = null;
        let accessPeriodRaw = null;
        if (periodoNum > 0 && periodoUnit) {
            if (periodoUnit === 'horas')
                accessPeriodDays = Math.round((periodoNum / 24) * 100) / 100;
            else if (periodoUnit === 'dias')
                accessPeriodDays = periodoNum;
            else if (periodoUnit === 'meses')
                accessPeriodDays = periodoNum * 30;
            accessPeriodRaw = `${periodoNum} ${periodoUnit}`;
        }
        if (accessPeriodDays !== null && accessPeriodDays > 90) {
            let autoRejectRequesterId;
            try {
                ...
                if (email) {
                    ...
                    if (userDb)
                        autoRejectRequesterId = userDb.id;
                }
            }
            catch (_) { }
            const { registrarMudanca } = await Promise.resolve().then(() => __importStar(require('../lib/auditLog')));
...
Caso precise de acesso permanente, entre em contato com o time de Segurança da Informação.`;
            await client.chat.postMessage({ channel: body.user.id, text: rejectMsg });
            return;
        }
        ...
        if (!toolName) {
            await client.chat.postMessage({ channel: body.user.id, text: '⚠️ Apenas Owners de ferramentas podem indicar um Deputy. Você não é proprietário de nenhuma ferramenta no momento.' });
            return;
        }
        const details = {
...
function buildNotificationSummary(requestType, detailsJson) {
    let d = {};
    try {
    ...
    if (requestType === 'INFRA_SUPPORT') {
        if (d.requestTypeLabel || d.requestType)
            parts.push(`Tipo: ${d.requestTypeLabel || d.requestType}`);
        if (d.urgencyLabel || d.urgency)
            parts.push(`Urgência: ${d.urgencyLabel || d.urgency}`);
        if (d.description)
            parts.push(`Descrição: ${d.description.slice(0, 150)}${d.description.length > 150 ? '…' : ''}`);
    }
    else if (PEOPLE_TYPES.includes(requestType) && requestType !== 'DEPUTY_DESIGNATION') {
        const collab = d.collaboratorName || d.info?.toString().replace(/^[^:]+:\s*/, '') || '—';
        parts.push(`Colaborador: ${collab}`);
        if (requestType === 'CHANGE_ROLE') {
            const curr = d.current;
            const fut = d.future;
            if (curr?.role || curr?.dept)
                parts.push(`Atual: ${[curr?.role, curr?.dept].filter(Boolean).join(' / ')}`);
            if (fut?.role || fut?.dept)
                parts.push(`Novo: ${[fut?.role, fut?.dept].filter(Boolean).join(' / ')}`);
        }
        if (requestType === 'HIRING' && (d.role || d.dept || d.startDate)) {
            if (d.role)
                parts.push(`Cargo: ${d.role}`);
            if (d.dept)
                parts.push(`Depto: ${d.dept}`);
            if (d.startDate)
                parts.push(`Início: ${d.startDate}`);
        }
        ...
        const sub = d.substituteName || d.substituteEmail || d.substitute || '—';
        ...
        if (d.beneficiary)
            parts.push(`Beneficiário: ${d.beneficiary}`);
        if (requestType === 'ACCESS_TOOL' && (d.owner || d.subOwner)) {
            if (d.owner)
                parts.push(`Owner: ${d.owner}`);
            if (d.subOwner)
                parts.push(`Sub-Owner: ${d.subOwner}`);
        }
    }
    const detailsText = parts.length ? parts.join(' · ') : d.info || '—';
    return { category, typeLabel, detailsText };
}
const sendSlackNotification = async (email, status, adminNote, requestType, ownerName, requestDetails) => {
    if (!slackApp)
        return;
    try {
