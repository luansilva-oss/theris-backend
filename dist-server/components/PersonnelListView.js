...
const PersonnelListView = ({ users, departments, roles, onEditUser, onDeleteUser, onEditDepartment, onDeleteDepartment, onEditRole }) => {
    ...
    const units = react_1.default.useMemo(() => {
    ...
    const getUsersByUnitDeptJob = (unit, dept, jobTitle) => users.filter(u => (u.unit || '').trim() === unit && u.department === dept && (u.jobTitle || '') === jobTitle);
    ...
    const getUsersByDeptJob = (deptName, jobTitle) => users.filter(u => u.department === deptName && (u.jobTitle || '').trim() === jobTitle);
    const showByUnit = units.length > 0;
    const fallbackDeptNames = react_1.default.useMemo(() => {
        if (showByUnit)
            return [];
        return [...new Set(users.map(u => u.department).filter(Boolean))];
    }, [showByUnit, users]);
    return ((0, jsx_runtime_1.jsx)("div", { className: "personnel-list-view", style: { display: 'flex', flexDirection: 'column', gap: '12px', paddingBottom: 24 }, children: showByUnit ? units.map(unit => {
            ...
            return ((0, jsx_runtime_1.jsxs)("div", { style: { border: '1px solid #1f1f22', borderRadius: '12px', overflow: 'hidden', background: '#09090b' }, children: [(0, jsx_runtime_1.jsxs)("div", { onClick: () => toggleUnit(unitKey), style: {
                        ...
                        }, children: [(0, jsx_runtime_1.jsxs)("div", { style: { display: 'flex', alignItems: 'center', gap: '12px' }, children: [(0, jsx_runtime_1.jsx)(lucide_react_1.Building2, { size: 22, color: "#8b5cf6" }), (0, jsx_runtime_1.jsxs)("span", { style: { fontWeight: 700, color: '#f4f4f5', fontSize: '16px' }, children: ["UNIDADE: ", unit] })] }), expandedUnits[unitKey] ? (0, jsx_runtime_1.jsx)(lucide_react_1.ChevronDown, { size: 20, color: "#71717a" }) : (0, jsx_runtime_1.jsx)(lucide_react_1.ChevronRight, { size: 20, color: "#71717a" })] }), expandedUnits[unitKey] && ((0, jsx_runtime_1.jsx)("div", { style: { padding: '12px 16px 16px', background: '#09090b' }, children: deptNames.map(deptName => {
        ...
        }) : ((0, jsx_runtime_1.jsxs)(jsx_runtime_1.Fragment, { children: [(0, jsx_runtime_1.jsx)("div", { style: { padding: '12px 16px', marginBottom: 16, background: '#18181b', borderRadius: 8, color: '#a1a1aa', fontSize: 13 }, children: "Nenhuma unidade definida nos colaboradores. Exibindo por departamento." }), fallbackDeptNames.length === 0 ? ((0, jsx_runtime_1.jsx)("div", { style: { padding: 24, textAlign: 'center', color: '#71717a', fontSize: 14 }, children: "Nenhum colaborador com departamento definido. Edite os colaboradores e informe unidade e departamento." })) : fallbackDeptNames.map(deptName => {
                    const usersInDept = users.filter(u => u.department === deptName);
                    const jobTitles = [...new Set(usersInDept.map(u => (u.jobTitle || '').trim()).filter(Boolean))];
                    const deptKey = `fallback-${deptName}`;
                    const deptEntity = findDepartment(deptName);
                    return ((0, jsx_runtime_1.jsxs)("div", { style: { marginBottom: 12, border: '1px solid #27272a', borderRadius: '10px', overflow: 'hidden', background: '#09090b' }, children: [(0, jsx_runtime_1.jsxs)("div", { style: {
                                    padding: '12px 16px',
                                    background: '#18181b',
                                    display: 'flex',
                                    alignItems: 'center',
