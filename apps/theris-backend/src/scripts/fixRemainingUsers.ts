...
async function main() {
  ...
  const deptComercialContact = await prisma.department.findFirst({
    where: { name: { contains: 'Comercial Contact', mode: 'insensitive' } }
  })
  let roleSalesOps = await prisma.role.findFirst({
    where: { name: 'SalesOps' }
  })
  if (!roleSalesOps && deptComercialContact) {
    roleSalesOps = await prisma.role.create({
      data: {
        name: 'SalesOps',
  ...
  const deptDizify = await prisma.department.findFirst({
    where: { name: { contains: 'Dizify', mode: 'insensitive' } }
  })
  let roleLiderDizify = await prisma.role.findFirst({
    where: { name: 'Líder de Vendas Dizify' }
  })
  if (!roleLiderDizify && deptDizify) {
    roleLiderDizify = await prisma.role.create({
      data: {
  ...
  for (const email of emailsParaDeletar) {
    const u = await prisma.user.findFirst({ where: { email: { contains: email } } })
    if (u) {
  ...
  for (const nome of nomesParaDeletar) {
    const u = await prisma.user.findFirst({ where: { name: nome } })
    if (u) {
      await prisma.user.delete({ where: { id: u.id } })
  ...
  const atribuicoes: Array<{ nome: string; roleId: string }> = [
  ...
  for (const { nome, roleId } of atribuicoes) {
    const user = await prisma.user.findFirst({ where: { name: nome } })
    if (!user) { console.log(`⚠️  Não encontrado: ${nome}`); continue }
